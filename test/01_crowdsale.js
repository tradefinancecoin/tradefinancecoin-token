...
contract('TFCToken', function(accounts) {
  ...
  var tfcTeamAddr1 = web3.eth.accounts[7]; // wings dao, bounties
  var tfcTeamAddr2 = web3.eth.accounts[9]; // vested tokens
  var tfcFundAddr = web3.eth.accounts[8];
  ...
  it("initialize contract", function() {
    ...
    .then(function(startDate) {
      ...
      return TFCToken.new(
        ownerAddr, // multisig
        tfcTeamAddr1, // team, whre 2% wings and 2% bounty will be received
  ...
  it('call grantVested()', () => {
    var start;
    return crowdsale.grantVested(tfcTeamAddr2, tfcFundAddr, { from: ownerAddr })
   .then(function() { 
    return crowdsale.balanceOf(tfcTeamAddr2)
   }).then(function(b) {
  ...
  it('vesting schedule - check cliff & vesting afterwards (advances time)', () => {
    ...
    return crowdsale.transfer(recepient, afterCliffAmount, { from: tfcTeamAddr2 })
    ...
    .then(function() {
      ...
      return crowdsale.transfer(recepient, amount + 10*10000, { from: tfcTeamAddr2 })
      .then(function() { 
        throw new Error('should not be here - allowed to transfer - 2') 
      })
      .catch(function(err) {        
        assert.equal(err.message, 'VM Exception while processing transaction: invalid opcode')
        return crowdsale.transfer(recepient, amount, { from: tfcTeamAddr2 })
      })
    })
    .then(function() {
      return crowdsale.balanceOf(recepient)
